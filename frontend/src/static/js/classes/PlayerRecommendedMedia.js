...
function itemThumb( img, duration ){
	return img ? '<span class="more-media-item-thumb" style="background-image:url(\'' + img + '\');">' + itemDuration( duration ) + '</span>' : '';
}
...
function gridClassname(itemsLength, container){
	...
	const pr = pw / ph;
	let ret = matchedRatioGrids( generateRatiosGrids( [1,2,3,4,5,6], [1,2,3,4,5,6], itemsLength, parseInt( ww, 10 ), parseInt( wh, 10 ) ), ww, wh, pw, ph, pr, 250 );
...
function buildItemsElements( itemsData, items, wrapper, inEmbed, hideViews ){
	...
	while(i<itemsData.length){
		...
		items[i].innerHTML = '<a href="' + itemsData[i].url + '" title="'+ itemsData[i].title + '"' + ( inEmbed ? 'target="_blank"' : "" ) + '>'
						   + itemThumb( itemsData[i].thumbnail_url, _MediaDurationInfo.toString() )
						   + itemContent( itemsData[i].title, itemsData[i].author_name, formatNumber( itemsData[i].views ) + ' ' + ( 1 >= itemsData[i].views ? 'view' : 'views' ), hideViews )
						   + '</a>';
...
export default function PlayerRecommendedMedia( itemsData, inEmbed, hideViews ){
	...
	domElems.prevSlide.innerHTML = '<button class="circle-icon-button"><span><span><i class="vjs-icon-navigate-before"></i></span></span></button>';
	domElems.nextSlide.innerHTML = '<button class="circle-icon-button"><span><span><i class="vjs-icon-navigate-next"></i></span></span></button>';
