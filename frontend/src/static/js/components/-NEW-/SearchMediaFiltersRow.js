...
function MediaFilterOption(props){
	...
	return (<div className="media-filter-option"><button className={ props.selected ? 'active' : '' } onClick={ onSelectOption }>{ props.label }</button></div>);
}
		...
		while( i < props.items.length ){
			...
			if( props.hideOptionOnSelect ){
				...
				if( props.items[i].id !== selected ){
					ret.push( <MediaFilterOption key={ props.items[i].id } id={ props.items[i].id } label={ props.items[i].label } selected={ props.items[i].id === selected } onSelect={ onSelectOption } /> );
				}
...
function MediaTypesFilter(props){
	...
	return (<div ref={ containerRef } className="media-filter">
				...
				<PopupContent contentRef={ popupContentRef }>
					<div className="main-options">
						<PopupMain>
							<MediaFilterOptionsList items={props.options} default={value} onSelect={setValue} hideOptionOnSelect={props.hideOptionOnSelect} />
						</PopupMain>
					</div>
				</PopupContent>
...
function MediaSortingFilter(props){
	...
	return (<div ref={ containerRef } className="media-filter">
				...
				<PopupTrigger contentRef={ popupContentRef }>
					<button className="popup-trigger" aria-label="Filter"><MaterialIcon type="sort" /><span className="filter-button-label"><span className="filter-button-label-text">{ props.label }</span></span></button>
				</PopupTrigger>
...
export function SearchMediaFiltersRow(props){
	...
	function onSelectFilterTypeOption( newFilterTypeId ){ setSelectedFilterTypeId(newFilterTypeId); }
	function onSelectSortOption( newSortId ){ setSelectedSortId(newSortId); }
	...
	return (<div className="media-filters-row">
