...
export function ManageMediaItem(props){
	...
	return (<div className="item manage-item manage-media-item">
				...
				<div  className="mi-title">
					...
					{ props.hideDeleteAction ? null : <ManageItemMediaActions containerRef={actionsContainerRef} title={props.title} onProceed={onClickProceed} /> }	
				</div>
				<div className="mi-added"><ManageItemDate date={ props.add_date } /></div>
				<div className="mi-author"><ManageItemMediaAuthor name={ props.author_name } url={ props.author_url } /></div>
				...
				<div className="mi-reviewed">{ void 0 === props.is_reviewed ? <i className="non-available">N/A</i> : ( props.is_reviewed ? <MaterialIcon type="check_circle" /> : <MaterialIcon type="cancel" /> ) }</div>
				<div className="mi-featured">{ void 0 === props.featured ? <i className="non-available">N/A</i> : ( props.featured ? <MaterialIcon type="check_circle" /> : '-' ) }</div>
				<div className="mi-reported">{ void 0 === props.reported_times ? <i className="non-available">N/A</i> : ( 0 === props.reported_times ? <span>-</span> : <span className="reported-number">{ props.reported_times } { 'time' + ( 1 < props.reported_times ? 's' : '' ) }</span> ) }</div>
			</div>);
}
