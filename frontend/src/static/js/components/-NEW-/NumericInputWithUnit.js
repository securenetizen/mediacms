...
export function NumericInputWithUnit(props){
	...
	function unitOptions(){
		...
		while( i < props.units.length ){
			...
			if( void 0 !== props.units[i].key ){
				ret.push( <option key={ props.units[i].key } value={ props.units[i].key }>{ void 0 !== props.units[i].label ? props.units[i].label : props.units[i].key }</option> );
			}
	...
	return (<div className="num-value-unit">
				...
				<select ref={ valueUnitRef } className="value-unit" onChange={ onChangeUnit } value={ null !== currentUnit ? currentUnit : '' }>{ unitOptions() }</select>
		   	</div>);
}
