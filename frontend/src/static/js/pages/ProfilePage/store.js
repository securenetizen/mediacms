    ...
    actions_handler(action) {
        switch(action.type) {
            case 'REMOVE_PROFILE':
                ...
                deleteRequest( deleteAPIurl, { headers: { 'X-CSRFToken': getCSRFToken() } }, false, this.removeProfileResponse, this.removeProfileFail );
                break;
            case 'LOAD_AUTHOR_DATA':
                getRequest( this.mediacms_config.api.users + '/' + window.MediaCMS.profileId, !1, this.onDataLoad, this.onDataLoadFail );
                break;
        }
    }
}
